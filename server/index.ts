...
async function startTurn(
  ...
  void (async () => {
    try {
      ...
      // Custom remote MCP servers (HTTP/SSE, enabled servers only)
      const remoteMcp = httpMcpServers(cfg).filter((s) => s.enabled !== false);
      if (remoteMcp.length) integrations.mcpServers = remoteMcp;
      // user-configured stdio MCP servers (config.json record form): same rule
      // as composio — only to a driver that can mount them. Their tools are
...
function configStatus() {
  return {
    ...
    mcpServers: httpMcpServers(cfg).map((s) => ({
      name: s.name,
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    // LAN mode (OMB_AUTH_TOKEN / OMB_HOST 0.0.0.0 / CIDR bypass) is the explicit
    // exception: the Host is a LAN address and the origin is whatever CORS_ORIGIN
    // allows. Pairing/session auth still applies when LAN mode is off.
    ...
    if (originHeader && !isAllowedOrigin(originHeader)) {
      ...
      if (CORS_ORIGIN && CORS_ORIGIN !== "*" && originHeader !== CORS_ORIGIN) {
        return json(res, 403, { error: "forbidden: cross-origin request" });
      }
    }
    ...
    // code into a session. Everything else needs the loopback owner, a paired
    // session, or LAN token/CIDR bypass.
    ...
    if (path.startsWith("/api/internal/")) {
      auth = { kind: "loopback", scopes: ["admin", "client"] as const };
    } else if (lanMode) {
    ...
    if (m && method === "DELETE") {
      ...
      try {
        ...
        if (directClaim?.threadId) allThreadIds.add(directClaim.threadId);
    ...
    if (method === "POST" && path === "/api/mcp/verify") {
      ...
      const savedHeaders = name ? httpMcpServers(cfg).find((s) => s.name === name)?.headers : undefined;
