    ...
    >
      ...
      >
        ...
        <div ref={tailRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto px-5 py-4">
          ...
          {group &&
            messages.map((m) => (
              <CommPopupRow key={m.id} message={m} threadId={group.threadId} />
            ))}
          {group?.busyBotId && (
...
function CommPopupRow({ message, threadId }: { message: Message; threadId: string }) {
  if (message.kind === "activity" && message.tool) {
    return <ActivityChip message={message} threadId={threadId} />;
  }
