...
type VoiceEngine = "elevenlabs" | "system" | "openai-compatible";
...
export function VoiceSettings({
  ...
  const provider: VoiceEngine = tts?.provider ?? "elevenlabs";
  ...
  const setProvider = (next: VoiceEngine) => {
  ...
  const engineBlurb =
    provider === "system"
      ...
      : provider === "openai-compatible"
        ? " the OpenAI-compatible server (Kokoro, LiteLLM, …) is shared by the workspace."
          ...
          <div className="mt-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && baseUrl.trim() && void saveCredentials()}
