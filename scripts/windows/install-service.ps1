...
function Test-LoopbackBindHost([string]$BindHost) {
    $h = $BindHost.Trim().ToLowerInvariant()
    if ($h.StartsWith("[") -and $h.EndsWith("]")) { $h = $h.Substring(1, $h.Length - 2) }
    if ($h -eq "localhost" -or $h -eq "localhost.") { return $true }
    if ($h -eq "::1" -or $h -eq "0:0:0:0:0:0:0:1") { return $true }
    return $h -like "127.*"
}

# Off-machine bind without a token is refused — the harness will not start either.
if (-not (Test-LoopbackBindHost $BindAddress) -and [string]::IsNullOrWhiteSpace($AuthToken)) {
    Write-Error @"
Refusing to install: -Host $BindAddress is not loopback and -AuthToken is empty.
The harness exits if OMB_HOST is off-machine without OMB_AUTH_TOKEN.

Generate a token:
  `$token = [System.Convert]::ToBase64String([System.Security.Cryptography.RandomNumberGenerator]::GetBytes(32))
Then re-run with: -AuthToken `$token
"@
    exit 1
}
...
# One AppEnvironmentExtra call — a later set replaces the previous list.
$envVars = [System.Collections.Generic.List[string]]::new()
$envVars.Add("OMB_HOST=$BindAddress")
$envVars.Add("OMB_PORT=$Port")
if (-not [string]::IsNullOrWhiteSpace($AuthToken)) { $envVars.Add("OMB_AUTH_TOKEN=$AuthToken") }
if (-not [string]::IsNullOrWhiteSpace($CorsOrigin)) { $envVars.Add("OMB_CORS_ORIGIN=$CorsOrigin") }
& $nssmExe set $ServiceName AppEnvironmentExtra @($envVars.ToArray())
...
if ($service.Status -eq 'Running') {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri $url -TimeoutSec 5 -Headers $headers
