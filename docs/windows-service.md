...
### 1. LAN env (required for an off-machine bind)

The service sets `OMB_HOST`, `OMB_PORT`, `OMB_AUTH_TOKEN`, and `OMB_CORS_ORIGIN`. A non-loopback `-Host` **requires** `-AuthToken` — the installer exits, and the harness will refuse to start without the token. See [docs/headless-lan-access.md](./headless-lan-access.md).
